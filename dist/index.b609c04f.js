...
class Chart {
    constructor({ parent, data, centerRadius, strokeColor, textColor, fontSize, name, emptySliceColor, canvasColor, sliceRadius, emptySliceName }){
        this.parent = parent;
    ...
    checkInOrOutClick(x, y) {
        return Math.pow(this.centerRadius / 2, 2) - Math.pow(x - this.centerX / 2 - this.gapX, 2) - Math.pow(y - this.centerY / 2 - this.gapY, 2) > 0 || Math.pow(this.sliceRadius / 2, 2) - Math.pow(x - this.centerX / 2 - this.gapX, 2) - Math.pow(y - this.centerY / 2 - this.gapY, 2) < 0;
    }
    getStrokeFunctions() {
        let total = 0;
        this.data.forEach(({ percent }, i)=>{
            ...
            let startIs270MoreOr90Less = false, endIs270MoreOr90Less = false, isBigger180 = false;
            if (start <= Math.PI / 2 || start >= Math.PI * 1.5) startIs270MoreOr90Less = true;
            if (end <= Math.PI / 2 || end >= Math.PI * 1.5) endIs270MoreOr90Less = true;
            if (gapEndStart > Math.PI) isBigger180 = true;
            let k1 = Math.tan(start), b1 = centerY - k1 * centerX, k2 = Math.tan(end), b2 = centerY - k2 * centerX;
            let k2PerpendicularX = false;
            if (end.toFixed(5) === (Math.PI / 2).toFixed(5) || end.toFixed(5) === (Math.PI * 1.5).toFixed(5)) k2PerpendicularX = true;
            let k1PerpendicularX = false;
            if (start.toFixed(5) === (Math.PI / 2).toFixed(5) || start.toFixed(5) === (Math.PI * 1.5).toFixed(5)) k1PerpendicularX = true;
    ...
    createPopup() {
        document.querySelector("body").insertAdjacentHTML("afterbegin", `<div class="popup"></div>`);
    }
    pushEmptySlice(array) {
        const totalPercent = array.reduce((accumulator, { percent })=>{
            return accumulator + percent;
        }, 0);
        if (totalPercent < 100) array.push({
    ...
    mousemoveHandler(e) {
        ...
        for(let i = 0; i < this.data.length; i++){
            ...
            if (coll.k2PerpendicularX && coll.end.toFixed(5) === (Math.PI * 1.5).toFixed(5)) bottom = e.pageX < this.centerX / 2 + this.canvas.offsetLeft;
            ...
            if (coll.k1PerpendicularX && coll.start.toFixed(5) === (Math.PI * 1.5).toFixed(5)) top = e.pageX > this.centerX / 2 + this.canvas.offsetLeft;
    ...
    drawChart() {
        for (const { color, percent } of this.data){
            ...
            const intervalId = setInterval(()=>{
                ...
                this.ctx.arc(this.centerX, this.centerY, this.sliceRadius, sliceCurrentPercent * PERCENT_VALUE, (sliceCurrentPercent + angle) * PERCENT_VALUE);
                this.ctx.fill();
...
//# sourceMappingURL=index.b609c04f.js.map
